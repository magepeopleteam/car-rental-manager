	...
	if ( ! class_exists( 'MPCRM_Custom_Slider' ) ) {
		class MPCRM_Custom_Slider {
			...
			public function super_slider( $post_id = '', $meta_key = '' ) {
				$type      = MPCRM_Global_Function::mpcrm_get_slider_settings( 'slider_type', 'slider' );
			...
			public function slider( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$showcase_position = MPCRM_Global_Function::mpcrm_get_slider_settings( 'showcase_position', 'right' );
					$column_class      = $showcase_position == 'top' || $showcase_position == 'bottom' ? 'area_column' : '';
					$slider_style      = MPCRM_Global_Function::mpcrm_get_slider_settings( 'slider_style', 'style_1' );
					?>
					<div class="superSlider placeholder_area fdColumn">
							...
							<?php
								...
								if ( $slider_style == 'style_2' ) {
									...
									<div class="abTopLeft">
										...
										<?php 
											echo esc_html__( 'View All', 'car-rental-manager' ) . ' ' . esc_html( (string) sizeof( $image_ids ) ) . ' ' . esc_html__( 'Images', 'car-rental-manager' ); 
										?>
										</button>
									</div>
									<?php
						...
						<?php
							$slider_indicator = MPCRM_Global_Function::mpcrm_get_slider_settings( 'indicator_visible', 'on' );
							$icon             = MPCRM_Global_Function::mpcrm_get_slider_settings( 'indicator_type', 'icon' );
			...
			public function post_thumbnail( $image_id = '' ) {
				$thumbnail = MPCRM_Global_Function::mpcrm_get_image_url( '', $image_id );
				if ( $thumbnail ) {
					?>
					...
					<div class="sliderAllItem">
						<?php
							...
							foreach ( $image_ids as $id ) {
								$image_url = MPCRM_Global_Function::mpcrm_get_image_url( '', $id );
								?>
			...
			public function slider_showcase( $image_ids ) {
				$showcase = MPCRM_Global_Function::mpcrm_get_slider_settings( 'showcase_visible', 'on' );
					...
					if ( $count == 4 ) {
						...
						<div class="sliderShowcaseItem" data-target-popup="superSlider" data-placeholder>
							...
							<div class="sliderMoreItem">
								...
								<?php echo esc_html( (string) (sizeof( $image_ids ) - 4) ); ?>
				...
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					...
					<div class="slideIndicator">
						<?php
							$count = 1;
							foreach ( $image_ids as $id ) {
								$image_url = MPCRM_Global_Function::mpcrm_get_image_url( '', $id, array( 150, 100 ) );
								?>
			...
			public function slider_popup( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$popup_icon_indicator = MPCRM_Global_Function::mpcrm_get_slider_settings( 'popup_icon_indicator', 'on' );
					?>
					<div class="sliderPopup" data-popup="superSlider">
						<div class="superSlider">
							...
							<div class="popupFooter">
								<?php
									$indicator = MPCRM_Global_Function::mpcrm_get_slider_settings( 'popup_image_indicator', 'on' );
									if ( $indicator == 'on' ) {
										$this->image_indicator( $image_ids );
									}
								?>
			...
			public function mpcrm_get_slider_ids( $post_id, $key ) {
				...
				$image_ids = MPCRM_Global_Function::mpcrm_get_post_info( $post_id, $key, array() );
				if ( $thumb_id ) {

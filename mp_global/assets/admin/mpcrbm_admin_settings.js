function mpcrbm_load_sortable_datepicker(parent, item) {
    if (parent.find(".item_insert_before").length > 0) {
        jQuery(item)
            .insertBefore(parent.find(".item_insert_before").first())
            .promise()
            .done(function () {
...
(function ($) {
    ...
    //=========upload image==============//
    $(document).on("click", "div.mpcrbm .add_single_image", function () {
        let parent = $(this);
        ...
        wp.media.editor.send.attachment = function (props, attachment) {
            ...
            let html =
                '<div class="single_image_item" data-image-id="' +
                attachment_id +
    ...
    $(document).on("click", "div.mpcrbm .remove_single_image", function (e) {
        e.stopPropagation();
    ...
    $(document).on("click", "div.mpcrbm .remove_multi_image", function () {
    ...
    $(document).on("click", "div.mpcrbm .add_multi_image", function () {
        let parent = $(this).closest(".multi_image_area");
        wp.media.editor.send.attachment = function (props, attachment) {
            let attachment_id = attachment.id;
            let attachment_url = attachment.url;
            let html =
                '<div class="multi_image_item" data-image-id="' +
                attachment_id +
                '"><span class="fas fa-times circleIcon_xs remove_multi_image"></span>';
            html += '<img src="' + attachment_url + '" alt="' + attachment_id + '"/>';
            html += "</div>";
            parent.find(".multi_image").append(html);
    ...
    //=========Remove Setting Item ==============//
    $(document).on("click", "div.mpcrbm .item_remove", function (e) {
        e.preventDefault();
        if (
            confirm(
            ...
            $(this).closest(".remove_area").slideUp(250).remove();
            return true;
        } else {
            return false;
        }
    ...
    //=========Add Setting Item==============//
    $(document).on("click", "div.mpcrbm .add_item", function () {
        ...
        mpcrbm_load_sortable_datepicker(parent, item);
...
(function ($) {
    "use strict";
    //=================select icon=========================//
    $(document).on("click", "div.mpcrbm .add_icon_image_area button.icon_add", function () {
    ...
    $(document).on("click", "div.mpcrbm .add_icon_image_area .icon_remove", function () {
    ...
    //=================select Single image=========================//
    $(document).on("click", "div.mpcrbm button.image_add", function () {
        let $this = $(this);
        let parent = $this.closest(".add_icon_image_area");
        wp.media.editor.send.attachment = function (props, attachment) {
            let attachment_id = attachment.id;
    ...
    $(document).on("click", "div.mpcrbm .add_icon_image_area .image_remove", function () {
            let parent = $(this).closest(".add_icon_image_area");
            parent.find('input[type="hidden"]').val("");

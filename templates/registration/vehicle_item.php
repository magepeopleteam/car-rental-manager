...
$mpcrbm_fixed_time = $fixed_time ?? 0;
...
$mpcrbm_start_place = $mpcrbm_start_place ?? isset($_POST['start_place']) ? sanitize_text_field(wp_unslash($_POST['start_place'])) : '';
$mpcrbm_end_place = $mpcrbm_end_place ?? isset($_POST['end_place']) ? sanitize_text_field(wp_unslash($_POST['end_place'])) : '';
...
//    error_log( print_r( [ '$post_id' =>$post_id ,'$mpcrbm_get_stock_data' =>$mpcrbm_get_stock_data], true ) );
    ...
    $mpcrbm_brand_terms        = wp_get_post_terms($post_id, 'mpcrbm_car_brand');
    $mpcrbm_year_terms         = wp_get_post_terms($post_id, 'mpcrbm_make_year');

    $mpcrbm_maximum_bag        = get_post_meta($post_id, 'mpcrbm_maximum_bag', true);

    $mpcrbm_car_type_name   = !empty($mpcrbm_car_type_terms)  ? esc_html($mpcrbm_car_type_terms[0]->name)   : '—';
    $mpcrbm_fuel_type_name  = !empty($mpcrbm_fuel_type_terms) ? esc_html($mpcrbm_fuel_type_terms[0]->name)  : '—';
    $mpcrbm_seating_name    = !empty($mpcrbm_seating_terms)   ? esc_html($mpcrbm_seating_terms[0]->name)    : '—';
        ...
        <div class="mpcrbm-image-box">
            <div class="bg_image_area" data-placeholder>
                ...
                <!-- <?php if( $mpcrbm_ajax_search === 'yes' ){
                    ...
                    <img src="<?php echo esc_attr($mpcrbm_thumbnail); ?>">
                <?php }else{?>
                    <div data-bg-image="<?php echo esc_attr($mpcrbm_thumbnail); ?>"></div>
                    <img src="<?php echo esc_attr($mpcrbm_thumbnail); ?>">
                <?php }?> -->
            ...
            <div class=" mpcrbm_list">
                <div class="mpcrbm_car_specs_lists">
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-car"></i>
                        <div title="<?php echo esc_html_e('Car Type: ','car-rental-manager').esc_attr($mpcrbm_car_type_name); ?>">
                            <div class="spec-value"><?php echo esc_html($mpcrbm_car_type_name); ?></div>
                        </div>
                    </div>
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-gas-pump-alt"></i>
                        <div title="<?php echo esc_html_e('Fuel Type: ','car-rental-manager').esc_attr($mpcrbm_fuel_type_name); ?>">
                            <div class="spec-value"><?php echo esc_html($mpcrbm_fuel_type_name); ?></div>
                        </div>
                    </div>
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-bonus"></i>
                        <div title="<?php echo esc_html_e('Brands: ','car-rental-manager').esc_attr($mpcrbm_brand_name); ?>">
                            <div class="spec-value"><?php echo esc_html($mpcrbm_brand_name); ?></div>
                        </div>
                    </div>
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-time-quarter-to"></i>
                        <div title="<?php echo esc_html_e('Make Year: ','car-rental-manager').esc_attr($mpcrbm_year_name); ?>">
                            <div class="spec-value"><?php echo esc_html($mpcrbm_year_name); ?></div>
                        </div>
                    </div>
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-person-seat"></i>
                        <div title="<?php echo esc_html_e('Seating Capacity: ','car-rental-manager').esc_attr($mpcrbm_seating_name); ?>">
                            <div class="spec-value"><?php echo esc_html($mpcrbm_seating_name); ?></div>
                        </div>
                    </div>
                    <div class="mpcrbm_car_spec">
                        <i class="mi mi-person-luggage"></i>
                        <div title="<?php echo esc_html_e('Maximum Bags: ','car-rental-manager').esc_attr($mpcrbm_bag_count); ?>">

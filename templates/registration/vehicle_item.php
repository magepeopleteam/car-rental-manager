...
// Validate dates
$all_dates = MPCRBM_Function::mpcrm_get_date($post_id);
if (empty($all_dates) || !in_array($start_date, $all_dates, true)) {
    return;
}

// View settings
$mptbm_enable_view_search_result_page = MPCRBM_Global_Function::get_settings('mpcrbm_general_settings', 'enable_view_search_result_page');
...
if ($post_id) {
    // Get vehicle data
    $thumbnail = MPCRBM_Global_Function::mpcrm_get_image_url($post_id);
    $price = MPCRBM_Function::mpcrm_get_price($post_id, $start_place, $end_place, $start_date_time, $return_date_time);

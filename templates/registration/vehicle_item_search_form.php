...
$show_car_class = isset( $car_class ) ? $car_class : '';
...
if ($post_id) {
    ...
    $car_type_terms     = get_post_meta($post_id, 'mpcrbm_car_type', true);
    $fuel_type_terms    = get_post_meta($post_id, 'mpcrbm_fuel_type', true);
    $seating_terms      = get_post_meta($post_id, 'mpcrbm_seating_capacity', true);
    $brand_terms        = get_post_meta($post_id, 'mpcrbm_car_brand', true);
    $year_terms         = get_post_meta($post_id, 'mpcrbm_make_year', true);
    $maximum_bag        = get_post_meta($post_id, 'mpcrbm_maximum_bag', true);

    // --- Safely extract names (handles single or multiple terms) ---
    $car_type_name   = !empty($car_type_terms)  ? esc_html($car_type_terms[0])   : '—';
    $fuel_type_name  = !empty($fuel_type_terms) ? esc_html($fuel_type_terms[0])  : '—';
    $seating_name    = !empty($seating_terms)   ? esc_html($seating_terms[0])    : '—';
    $brand_name      = !empty($brand_terms)     ? esc_html($brand_terms[0])      : '—';
    $year_name       = !empty($year_terms)      ? esc_html($year_terms[0])       : '—';
    $bag_count       = !empty($maximum_bag)     ? esc_html($maximum_bag)               : '—';
    ?>
    <div class="mpcrbm_booking_vehicle <?php echo esc_attr($show_car_class);?> mpcrbm_booking_item <?php echo esc_attr('mpcrbm_booking_item_' . $post_id); ?> <?php echo esc_attr($hidden_class); ?> <?php echo esc_attr($feature_class); ?>" data-placeholder

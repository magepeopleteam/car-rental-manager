 ...
 */
    ...
    $end_locations = MPCRM_Function::get_end_location($start_place, $post_id);
    if (sizeof($end_locations) > 0) {
        ...
        <select class="formControl mptbm_map_end_place" id="mptbm_manual_end_place">
            ...
            <?php foreach ($end_locations as $location) { ?>
                <option value="<?php echo esc_attr($location); ?>"><?php echo esc_html(MPCRM_Function::get_taxonomy_name_by_slug( $location,'locations' )); ?></option>
            <?php } ?>
        </select>
    <?php } else { ?>

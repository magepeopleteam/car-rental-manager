			...
			public static function post_select() {
				...
				<label class="min_400 mptbm_post_id">
					<select name="mptbm_id" class="formControl mp_select2" id="mptbm_post_id" required>
						<option value="" selected><?php esc_html_e('Select', 'wpcarrently') . ' ' . esc_html($label); ?></option>
						<?php
							...
							foreach ($all_posts as $post) {
								...
								if ($post_id == $mptbm_id) {
									...
									//$price_based_text = $price_based == 'manual' ? esc_html__('Manual', 'wpcarrently') : esc_html__('Dynamic', 'wpcarrently');
									?>
